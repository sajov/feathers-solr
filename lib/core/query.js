...
function convertOperators (query, root = false, escapeFn) {
  if (Array.isArray(query)) {
    return query.map(q => {
      return convertOperators(q, false, escapeFn);
    });
  }
  ...
  const converted = Object.keys(query).reduce((res, prop) => {
    ...
    if (prop === '$or') {
      const o = [].concat.apply([], convertOperators(value, false, escapeFn));
      const {key, value} = escapeFn(root || prop, o);
      queryString = Operators.$or(key, value);
    } else if (_.has(Operators, prop)) {
      const {key, value} = escapeFn(root || prop, value);
      queryString = Operators[prop](key, value);
    } else if (typeof prop === 'string') {
      if (!_.isObject(value)) {
        const {key, value} = escapeFn(root || prop, value);
        queryString = Operators.$eq(key, value);
      } else {
        queryString = convertOperators(value, prop, escapeFn);
      }
      if (Array.isArray(queryString)) {
        if (queryString.length > 1) {
          const {key, value} = escapeFn(root || prop, queryString);
          queryString = Operators.$and(key, value);
        }
      }

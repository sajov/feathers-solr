...
function defineSchema(Service, schema) {
  ...
  return new Promise((resolve, reject) => {
    if (
      Service.options.migrate === "safe" ||
      Service.options.managedScheme === false ||
      _.isObject(Service.options.schema) === false
    ) {
      return reject();
    }
    ...
    if (Service.options.migrate === "drop") {
      Service.remove()
        .then(() => {
          ...
          schemaApi
            .deleteField(deleteSchemaFields(Service.options.schema)) ///
            .then(res => {
              debug("feathers-solr migrate reset schema", res);
              schemaApi
                .addField(describeSchemaFields(Service.options.schema))
                .then(function(res) {
      ...
      debug(
        "Schema Mode else:",
        Service.options.migrate,
        describeSchemaFields(Service.options.schema)
      );
      schemaApi
        .addField(describeSchemaFields(Service.options.schema))
        .then(function(res) {

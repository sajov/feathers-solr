...
class Service extends AdapterService {
  ...
  _patch(id, data, params = {}) {
    ...
    const ids =
      id === null
        ? this._getOrFind(id, params) //.then(mapIds)
        : Promise.resolve([id]);
      ...
      .then(idList => {
        ...
        const findParams = Object.assign(
          ...
          Object.assign(
            {},
            {
              query: Object.assign({ [this.id]: idList })
            }
          )
        );
        ...
        const getID = mapIds(idList);
        console.log("idList", idList, "idList");
        return this.Model.post("update/json", idList).then(() => {
          // return this._getOrFind(id, findParams).then(res => {

...
class Service extends AdapterService {
  ...
  _get(id, params = {}) {
    ...
    const query2 = { query: Object.assign({}, params.query) };
    return this.Model.post("query", utils.queryJson(id, query2)).then(res => {
  ...
  _find(params = {}) {
    ...
    const { filters, query, paginate } = this.filterQuery(params);
    const solrQuery = utils.solrQuery(null, filters, query, paginate);
    ...
    const queryJson = utils.queryJson(null, params, this.options);
    // console.log(queryJson);
    return this.Model.post("query", solrQuery).then(res =>
  ...
  _create(raw, params = {}) {
    ...
    const addId = item => utils.addId(item, this.id);

...
describe('Schema', () => {
  ...
  before(async () => {
    ...
    try {
      const res1 = await Client.post(`/${options.core}/schema`, {data: addSchema});
      console.log(res1)
    } catch (error) {
      console.log(error)
    }
  ...
  it('get `select`', async () => {
    // const {response} = await Client.post(`/${options.core}/schema`, {data: addSchema});
    const { response } = await Client.get(`/${options.core}/select`, { params: { "q": "*:*" } });
